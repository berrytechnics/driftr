 ...
 * Nyx pads (dust) slip Sol ↔ Vesper; a powered gate slips Vesper ↔ Cinder remnant.
 */
export const SYSTEM_IDS = {
  sol: 'sol',
  nyxAlt: 'nyxAlt',
  /** Sparse remnant pocket reachable only through the misplanted gate. */
  gateVoid: 'gateVoid',
} as const
export type SystemId = (typeof SYSTEM_IDS)[keyof typeof SYSTEM_IDS]

/**
 * Cool black dwarf at the hollow center of a supernova remnant —
 * the star died; builders later hung the matching gate on a tight orbit.
 */
export const VOID_STAR_NAME = 'Cinder'
export const VOID_SUN_SIZE = 34
export const VOID_SUN_COLOR = '#4a4248'
export const VOID_SUN_INTENSITY = 0.2
/** Soft residual gravity — enough to fall, not Sol-scale. */
export const VOID_MU = 1800
/** Gate semi-major radius around Cinder (world units). */
export const VOID_GATE_ORBIT = 268
export const VOID_GATE_ORBIT_INCLINATION = 0.22
/** Visual orbit rate (rad/s) — dramatic drift inside the remnant cavity. */
export const VOID_GATE_ORBIT_SPEED = 0.038
/**
 * Sparse remnant rubble beyond the gate’s orbit, into the nebula wall.
 * Occasional clumps stand in for worlds destroyed with the star.
 * No night-omen rocks — regular ore only if broken up.
 */
