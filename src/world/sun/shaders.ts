/** GLSL for the Tibi / fwdapps realistic sun (adapted for non-origin sun). */

export const visibilityGLSL = /* glsl */ `
...
export const simplex4dGLSL = /* glsl */ `
...
void main(void) {
    ...
    vNormal  = normalize(pWorld - sunWorld);
...
void main(void){
  ...
  pW += sideW * width;
  vNormal  = normalize(pW - sunWorld);
  float lenLocal = length(pOBJ);
  vOpacity  = smoothstep(R, R * 1.03, lenLocal);
